...
public class UIRoot : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        minimumHeight = maximumHeight = manualHeight = (int)(1280f * ((float)Screen.height / (float)Screen.width) / (1280f / 720f));
        float ratio = (float)Screen.height / (float)Screen.width;
        if (ratio < 1.49f)
        {
            minimumHeight = maximumHeight = manualHeight = (int)((float)Screen.width * 1.5f);
        }
    }
