...
static public class NGUIMath
{
    ...
    static public Bounds CalculateAbsoluteWidgetBounds(Transform trans, float scrollMaxY, UIWidget[] skipBoundObj)
    {
        if (trans != null)
        {
            ...
            for (int i = 0, imax = widgets.Length; i < imax; ++i)
            {
                ...
                bool skip = false;
                foreach (UIWidget obj in skipBoundObj)
                {
                    if (obj == w)
                    {
